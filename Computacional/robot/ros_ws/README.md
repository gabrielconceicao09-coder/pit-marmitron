...
Workspace ROS 2 usado nas simulações de navegação do Marmitron. É aqui que ficam os pacotes voltados à simulação do projeto e os arquivos paar a navegação autonoma do robô.
...
- A calibração de escala (`map_scale_calib`) é chamada de forma bloqueante durante o launch — a simulação só sobe o Nav2 depois que o serviço `calibrate_map_scale` responder. Se o EKF ou o SLAM não estiverem publicando corretamente, o launch IRÁ travar.
